...
export const SwapCard: React.FC<SwapCardProps> = ({
  ...
  const handleSwap = async () => {
    ...
    if (isDemoMode && !publicKey) {
      ...
      setTimeout(() => {
        ...
        recordActivity({
          type: "swap",
          title: `Swapped ${inAmount} ${inputToken.metadata?.symbol || "COOK"} for ${outputUiAmount} ${outputToken.metadata?.symbol || "bCOOK"}`,
          details: `Executed swap on Cookiebox DAMM v2 aggregator with sub-second finality`,
          amount: `${inAmount} ${inputToken.metadata?.symbol || "COOK"}`,
          txHash: sig,
          status: "confirmed",
        });
    ...
    try {
      ...
      recordActivity({
        type: "swap",
        title: `Swapped ${inAmount} ${inputToken.metadata?.symbol || "COOK"} for ${outputUiAmount} ${outputToken.metadata?.symbol || "bCOOK"}`,
        details: `On-chain swap via Nightly wallet on Cookie Chain SVM`,
        amount: `${inAmount} ${inputToken.metadata?.symbol || "COOK"}`,
        txHash: sig,
        status: "confirmed",
      });
      ...
      addToast({
        type: "error",
        title: "Swap Failed",
        message: err.message || "Failed to process transaction on Cookie Chain.",
      });
    } finally {
      setSwapping(false);
    }
  ...
  const handlePercentClick = (pct: number) => {
    const val = (userBalance * pct).toFixed(4);
    setInAmount(val);
  };

  return (
    <div className="w-full max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 items-start pb-6">
      {/* Left Column (7 cols): Pro Trading Chart + Live Trade Stream */}
      <div className="lg:col-span-7 flex flex-col gap-6">
        <ProTradingChart
          inputToken={inputToken as CookieToken}
          outputToken={outputToken as CookieToken}
          cookUsd={cookUsd}
        />
        <LiveTradeStream cookUsd={cookUsd} />
      </div>

      {/* Right Column (5 cols): Swap Execution Box */}
      <div className="lg:col-span-5 flex flex-col gap-4">
        {/* Order Type Tabs */}
        <div className="flex items-center bg-obsidian-900/90 rounded-2xl p-1.5 border border-white/[0.08] backdrop-blur-xl">
          {(["market", "limit", "dca"] as const).map((type) => (
            <button
              key={type}
            ...
            >
              {type === "market" ? "Market Swap" : type === "limit" ? "Limit Order" : "DCA Auto"}
            </button>
          ))}
        </div>

        {/* Main Swap Card */}
        <div className="p-5 sm:p-6 rounded-3xl bg-obsidian-900/90 border border-white/[0.08] shadow-card-subtle backdrop-blur-2xl relative overflow-hidden">
          {/* Header */}
          ...
          {showSettings && (
            <div className="p-3.5 mb-4 rounded-2xl bg-obsidian-950/80 border border-white/[0.06] animate-slide-in space-y-2">
            ...
            <div className="flex items-center gap-2 pt-1">
              {[0.25, 0.5, 0.75, 1.0].map((pct) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-obsidian-900 hover:bg-cookie-500/20 text-[10px] font-mono text-slate-400 hover:text-cookie-300 border border-white/[0.04] hover:border-cookie-500/30 transition-all"
                >
                  {pct === 1.0 ? "MAX" : `${pct * 100}%`}
                </button>
              ))}
